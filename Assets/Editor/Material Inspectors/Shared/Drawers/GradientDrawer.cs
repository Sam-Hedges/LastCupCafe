...
namespace GUIStreamline
{
    public class GradientDrawer : MaterialPropertyDrawer
    {
            ...
            using (var changeScope = new EditorGUI.ChangeCheckScope())
            {
                ...
                if (contextButtonClicked)
                {
                    ...
                    menu.AddItem(new GUIContent("Paste XML from Adobe Color"), false, () =>
                    {
                        ...
                        {
                            Debug.LogWarning(
                                "Clipboard contains a CSS gradient, which can not be reproduced in Unity. " +
        ...
        {
            var textureAsset =
                new Texture2D(_resolution, 1, _hdr ? TextureFormat.RGBAHalf : TextureFormat.ARGB32, false)
                {
