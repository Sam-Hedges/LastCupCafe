...
public class GravityManager : MonoBehaviour {
    ...
    private void OnTriggerEnter(Collider coll) {
        if (coll.CompareTag("GravityZone")) {
            ChangeGravityDirection(coll.transform);
        }
    }

    private void OnTriggerExit(Collider coll) {
        if (coll.CompareTag("GravityZone")) {
            ResetGravityDirection(coll.transform);
        }
    }
    ...
    private void ChangeGravityDirection(Transform newTransform) {
        ...
        playerRigidbody.rotation =
            Quaternion.LookRotation(Quaternion.Euler(90, 0, 0) * newTransform.up, newTransform.up);
    ...
    private void ResetGravityDirection(Transform newTransform) {
        ...
        if (Physics.gravity / gravityStrength != lastGravityVector) {
            return;
        }
