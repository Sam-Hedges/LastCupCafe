...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void GroundedCheck()
        {
            ...
            Grounded =
                Physics.CheckSphere(spherePosition, GroundedRadius, GroundLayers,
        ...
        private void Move()
        {
            // set target speed based on move speed, sprint speed and if sprint is
            // pressed
            float targetSpeed = _input.sprint ? SprintSpeed : MoveSpeed;

            // a simplistic acceleration and deceleration designed to be easy to
            // remove, replace, or iterate upon

            // note: Vector2's == operator uses approximation so is not floating
            // point error prone, and is cheaper than magnitude if there is no
            // input, set the target speed to 0
            ...
            {
                // creates curved result rather than a linear one giving a more
                // organic speed change note T in Lerp is clamped, so we don't need
                // to clamp our speed
                _speed =
                    Mathf.Lerp(currentHorizontalSpeed, targetSpeed * inputMagnitude,
            ...
            // note: Vector2's != operator uses approximation so is not floating
            // point error prone, and is cheaper than magnitude if there is a move
            // input rotate player when the player is moving
            if (_input.move != Vector2.zero)
            {
        ...
        private void JumpAndGravity()
        {
            if (Grounded)
            {
                ...
                {
                    // the square root of H * -2 * G = how much velocity needed to
            ...
            // apply gravity over time if under terminal (multiply by delta time
            // twice to linearly speed up over time)
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            // when selected, draw a gizmo in the position of, and matching radius
            // of, the grounded collider
        ...
        private void OnLand(AnimationEvent animationEvent)
        {
            ...
            {
                AudioSource.PlayClipAtPoint(
                    LandingAudioClip, transform.TransformPoint(_controller.center),
                    FootstepAudioVolume);
            }
        }
    }
